...
(defn run-ccat []
  ...
  (binding [ccat (init-ccat)
            quit-program false]
    (while (not quit-program)
           (set! ccat (update-everything ccat)
             (if (-> ccat :coderack empty?)
               (def ccat 
                    (assoc-in ccat [:slipnet :nodes]
                              (for [node (-> ccat :slipnet :nodes)]
